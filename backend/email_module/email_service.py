...
from email_module.config import mail_config
...
class EmailService:
    ...
    async def send_email(
        ...
        """
        Asynchronously sends HTML email using FastMail templates with defensive fallback logging.
        """
    ...
    async def send_welcome_email(user_email: str, name: str, company: str = "Enterprise"):
        try:
            ...
            logger.warning(f"send_welcome_email fallback: {e}")
    ...
    async def send_audit_completed_email(user_email: str, audit_data: dict):
        try:
            ...
            logger.warning(f"send_audit_completed_email fallback: {e}")
    ...
    async def send_high_risk_alert_email(user_email: str, audit_data: dict):
        try:
            ...
            logger.warning(f"send_high_risk_alert_email fallback: {e}")
    ...
    async def send_audit_failed_email(user_email: str, audit_id: int, error_msg: str):
        try:
            ...
            logger.warning(f"send_audit_failed_email fallback: {e}")
